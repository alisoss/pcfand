...
type  string22 = string[22];
      types=(H1,H2,H5,H11,D1,D2,D3,D5,B,U1,H127);
...
procedure WorkPlace ;
   ...
   begin MyWindow(1,2,80,TxtRows-2,_abs);  ClrScr ;
         ...
         writeln(' PC FAND '+ver+'- install program');
         CfgVyp:=CfgPathFull ;
  ...
  begin case t of H1:begin s:=hexB(pH1); EditLine(s,1,2,2,_hex);
                 ...
                 H11:begin s:=''; for i:=1 to length(pH11) do s:=s+HexB(byte(pH11[i]));
                  ...
                  D5:begin str(pD5:5,s); i:=1; while s[i]=' ' do delete(s,1,1);
                   ...
                   B:begin if pB=true then s:='Y' else s:='N'; EditLine(s,1,1,1,_bool);
                           pB:=s='Y';
                     end;
  ...
  begin case t of H1:begin s:=hexB(pH1) end;
                 ...
                 H127:begin s:=''; for i:=1 to length(pH127) do s:=s+HexB(byte(pH127[i]));
                  ...
                  D1:begin str(pD1:1,s) end;
                   B:begin if pB=true then s:='Y' else s:='N' end;
                  U1:begin s:=pU1 end;
        end;
        clreol; write(s);
  end;
...
const sNames:array[1..sItems] of string[38]=
                      ({'        Pocet vet k ulozeni v editoru',}
                       'Record number to save in editor',
	                       {'Sirka',}
                       '- width',
	                       {'Logicka delka',}
                       '- size',
...
const vItems=6;
const vNames:array[1..vItems] of string22=
            (	{'Adresa videopameti',}
             '  Video memory address',
	             {'Pocet radku',}
             'Number of monitor rows',
	             {'Osetrit snezeni ?',}
             '       Snow checking ?',
	             {'Zapnuty kurzor',}
             '    Cursor switched on',
...
const FItems=3 ;
const fNames:array[1..fItems] of string22=
             (	{'Kod',}
              'Code',
	              {'Pouzit fonty FANDu',}
              '   Use PC FAND fonts ?',
	              {'Monitor bez diakr.'}
              'Monitor without diacr.');
...
const pItems=32;
const pNames:array[1..pItems] of string[21]=
                        ({'^S Podtrzeni zap','Podtrzeni vyp',
			  '^W Kurziva   zap','Kurziva   vyp',
			  '^Q Siroky    zap','Siroky    vyp',
			  '^D Dvojity   zap','Dvojity   vyp',
			  '^B Mastny    zap','Mastny    vyp',
			  '^E Zhusteny  zap','Zhusteny  vyp',
			  '^A Elite     zap','Elite     vyp',}
                         '^S Underline      on','   Underline     off',
                         '^W Italic         on','   Italic        off',
                         '^Q Double-width   on','   Double-width  off',
                         '^D Double-strike  on','   Double-strike off',
                         '^D Double-strike  on','   Double-strike off',
                         '^E Compressed     on','   Compressed    off',
                         '^A Elite          on','   Elite         off',
	                         {'Reset tiskarny',}
                         '       Printer reset',
	                         {'Delka strany',}
                         '           Page size',
                         ...
                         '         Left margin',
	                         {'Uk.odsaz.zleva',}
                         '  End of left margin',
                         {'^X Uziv.1    zap','Uziv.1    vyp',
                          '^V Uziv.2    zap','Uziv.2    vyp',
                          '^T Uziv.3    zap','Uziv.3    vyp',}
                         '^X User code 1    on','   User code 1   off',
                         '^V User code 2    on','   User code 2   off',
                         '^T User code 3    on','   User code 3   off',
	                         {'Radkovani n/72"' ,}
                         ' Vert.spacing n/72" ',
	                         {'Radkovani n/216"', Tisk grafu}
                         'Vert.spacing n/216" ',
	                         {'Gr.hustota 60'   ,}
                         '       Gr.density 60',
	                         {'Gr.hustota 120',}
...
procedure sInst;
  begin MyWindow(1,2,50,TxtRows-2,_abs); Frame(_single,'','');
        ...
        with sCodes do begin write(sNames[1]:38,' : ');Edit(D3,UpdCount);Disp(D3,UpdCount);writeln;
                             ...
                             writeln; writeln('GRAPHIC PAGE DIVIDING':38) ;
                             ...
                             writeln; writeln('MOUSE SUPPORT':38);
                           ...
                           { write(sNames[27]:38,' : ');Edit(D2,CtrlDelay);Disp(D2,CtrlDelay);writeln;}
                             ...
                             writeln(#7,'End of constans - press ESC');
                             while ReadKey<>#27 do ;
                       end;
        writeln;
        WorkPlace;
...
procedure vInst;
  ...
  begin if cCurr=@cMono then s:='Monochrom' else
        ...
        writeln(#7,'End of monitor setting - press ESC');
        while ReadKey<>#27 do ;
  end;
...
procedure aInst;
  ...
  begin mm.PopUp:=true; mm.x:=$FF; mm.y:=$FF; mm.StrUp:='Alphabet';
        PushMenu(s[1]);
...
procedure pInst ;
  ...
  const st:ScreenStr='Standard (LPT1-4)/"logical" port (dos.LPT1-9)/Other port (print manager)//';
  var   i,m,tmout:word; s:string;
  begin
	        {mm.PopUp:=true; mm.x:=$FF; mm.y:=$FF; mm.StrUp:='Zpusoby tisku';}
        mm.PopUp:=true; mm.x:=$FF; mm.y:=$FF; mm.StrUp:='Print method';
        PushMenu(st[1]);
        {for i:=2 to aCode do AppKeyBuff(#0+_down);}
        ...
        if Printers[j].pTmOut=253 then begin
            ...
            write('  Name of the copy:'); s:=printers[j].pCodes[15];
            EditLine(s,1,255,255,_any); printers[j].pCodes[15]:=s; writeln;
	            {write('  Nazev programu:'); s:=printers[j].pCodes[16];}
            write('      Program name:'); s:=printers[j].pCodes[16];
            EditLine(s,1,255,255,_any); printers[j].pCodes[16]:=s; writeln;
	            {write(' Param. programu:'); s:=printers[j].pCodes[17];}
            write('Program parameters:'); s:=printers[j].pCodes[17];
            EditLine(s,1,255,255,_any); printers[j].pCodes[17]:=s; writeln;
            end
        else begin
          gotoxy(17,WhereY); write(' Typ: '); s:='M' ;
          repeat gotoxy(22,WhereY); s[1]:=printers[j].pType; EditLine(s,1,1,1,_any); printers[j].pType:=s[1];
          until (printers[j].pType in ['M','C','L']) ;
          gotoxy(22,WhereY); writeln(printers[j].pType);
          write('   Code(KL,KN,LK,LN):');
          ...
          if s='LK' then printers[j].pCode:='L' else if s='LN' then printers[j].pCode:='l' else printers[j].pCode:=' ';
	          ...
	          {write('      Cislo portu: ');}
          write('         No. of port: ');
          repeat gotoxy(24,WhereY); Edit(D1,Printers[j].pNo);Disp(D1,Printers[j].pNo);
        ...
        writeln(#7,'End of printer settings - press ESC');
        while ReadKey<>#27 do ;
        end;
  ...
  repeat
    ...
    LastLine(' (Enter)  (Esc)  ('+upCh+')  ('+downCh+
             ')   (Ins) new printer    (Del) delete   (Ctrl'+leftCh+') (Ctrl'+rightCh+') change order');
    MyWindow(1,2,80,TxtRows-2,_abs); Frame(_single,'','');
	    {writeln(' PREHLED INSTALOVANYCH TISKAREN:');}
    writeln(' LIST OF INSTALED PRINTERS:');
    SavedAttr:=TextAttr;
    ...
    LastLine('  use keys  (Enter)  (Esc)  ('+leftCh+')  ('+rightCh+')   you can edit inside window');
    case kl of
        #27  : konec:= true ;
        ...
        #82  : begin if prMax<10 then begin
                     ...
                     else message($FF,$FF,'max. 10 printers'); end ;
        #83  : begin if prMax>1 then begin
	                    {if verify($FF,$FF,'Opravdu zrusit tiskarnu ?') then begin}
                    if verify($FF,$FF,'Delete printer realy ?') then begin
               ...
               else message($FF,$FF,'You can not delete last printer!'); end ;
      end ;
  until konec ;
...
procedure MainMenu;
  ...
  const s1:ScreenStr='/Printers/Constants/Alphabet/Monitor/End//';
	        {s2:ScreenStr='S nahranim zmen na disk/Beze zmen//';}
        s2:ScreenStr='Save changes to disk/Not save//';
  var m:word;
  begin mm.x:=1; mm.y:=1; PushMenu(s1[1]);
        repeat beep; m:=menu; MenuOff;
               case m of { 1:cInst; }
                         ...
                         5:begin mm.x:=$FF; mm.y:=$FF; mm.StrUp:='End';
                                 PushMenu(s2[1]);
                                 m:=menu;
